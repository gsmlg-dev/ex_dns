defmodule DNS.Message.Record.Data.RRSIGTest do
  ...
  describe "new/1" do
    test "creates RRSIG record with valid parameters" do
      type_covered = DNS.ResourceRecordType.new(1) # A record
      algorithm = 8 # RSA/SHA-256
      labels = 2
      original_ttl = 3600
      signature_expiration = 1_672_531_200 # 2023-01-01 00:00:00 UTC
      signature_inception = 1_672_464_000 # 2022-12-31 00:00:00 UTC
      key_tag = 12345
  ...
  describe "DNS.Parameter protocol" do
    test "converts RRSIG record to iodata" do
      ...
      signers_name_binary = DNS.to_iodata(signers_name)
      expected_size = 2 + 1 + 1 + 4 + 4 + 4 + 2 + byte_size(signers_name_binary) + byte_size(signature)
      
      expected = <<expected_size::16, type_covered.value::binary, algorithm::8, labels::8, 
                   original_ttl::32, signature_expiration::32, signature_inception::32,
                   key_tag::16, signers_name_binary::binary, signature::binary>>

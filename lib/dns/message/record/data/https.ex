defmodule DNS.Message.Record.Data.HTTPS do
  @moduledoc """
  DNS HTTPS Record (Type 65)

  The HTTPS record is a specialized SVCB record for HTTPS services.
  It has the same format as SVCB but is specifically for HTTPS endpoints.
  ...
  defstruct raw: nil, type: RRType.new(65), rdlength: nil, data: nil
  ...
  defimpl DNS.Parameter, for: DNS.Message.Record.Data.HTTPS do
    @impl true
    def to_iodata(%DNS.Message.Record.Data.HTTPS{data: data}) do
  ...
  defimpl String.Chars, for: DNS.Message.Record.Data.HTTPS do
    def to_string(%DNS.Message.Record.Data.HTTPS{data: data}) do
      ...
      if byte_size(svc_params) == 0 do
        ...
        "#{svc_priority} #{target_name} #{parsed_params}"
      end
    end
